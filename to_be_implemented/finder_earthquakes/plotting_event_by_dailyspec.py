...
FIRSTDAY = '2020,03,25,11,00,00'
LASTDAY = '2020,03,25,11,59,00'
...
def create_axes(ratio_LF_height=0.6, figsize=figsize):
    """
    ...
    ax_cb = fig.add_axes([w_base + w_spec + w_psd / 2.,
                          ...
                          h_seis * 0.8],
                         label='colorbar')
    return fig, ax_cb, ax_psd_HF, ax_psd_LF, ax_seis_HF, ax_seis_LF,ax_spec_HF, ax_spec_LF
...
def format_axes(ax_cb: plt.Axes,
    ...
    ax_seis_LF.set_ylabel('%s \n[µm/s²]' % st_LF[0].get_id(), color='grey')
    ...
    mins = mdates.MinuteLocator(interval=1)
    # format the ticks
...
# Colorbar axis
ax_cb = fig.add_axes([w_base + w_spec + w_psd / 2.,h_base + h_spec_HF + h_spec_LF + h_seis * 0.1, w_psd * 0.2, h_seis * 0.8], label='colorbar')
...
for st, ax_spec, ax_psd, flim, winlen in zip([st_LF, st_HF],[ax_spec_LF, ax_spec_HF],[ax_psd_LF, ax_psd_HF],[(fmin_LF, fmax_LF),(fmin_HF, fmax_HF)], [winlen_LF, winlen_HF]):
    for tr in st:
        ...
        ax_psd.plot(10 * np.log10(median), f[bol],color='k',lw=0.25)
        ax_psd.plot(10 * np.log10(perc_95), f[bol],color='r', linestyle='dashed',lw=0.25)
        ax_psd.plot(10 * np.log10(perc_01), f[bol],color='darkgrey', linestyle='dotted',lw=0.25)
