...
markes_file = sorted(glob(OUTPUT_EV_DIR+'Local_Picked/*.txt'))
...
for mid,mfile in enumerate(markes_file):
	...
	for pick in picks_markers:
		...
		dic = {
				...
				'event_name': datetime_UTC.datetime.strftime('%d_%m_%Y_%H'),
				}
		dic_lst.append(dic)
...
for d in tqdm(event_dic_lst,desc='Dic loop'):
	...
	for sta in station_lst:
		...
		datetime_event_hour = df_P['datetime'].dt.strftime('%H').values[0]
		datetime_event_minute = df_P['datetime'].dt.strftime('%M').values[0]
		datetime_event_second = df_P['datetime'].dt.strftime('%S').values[0]
		datetime_event_microsecond = df_P['datetime'].dt.strftime('%f').values[0]

		event_time_UTCdattime = UTCDateTime(year=int(datetime_event_year), julday=int(datetime_event_julday), hour=int(datetime_event_hour), minute=int(datetime_event_minute),second=int(datetime_event_second),microsecond=int(datetime_event_microsecond))
		# ------------------------------------------
		# Retrieving mseed files
		sta_ev_name = df_sta['event_name'].values[0]

		station_file_HHZ = read(DIR_DATA+datetime_event_year+'/'+df_sta['network'].values[0]+'/'+sta+'/HHZ.D/'+df_sta['network'].values[0]+'.'+sta+'..HHZ.D.'+datetime_event_year+'.'+datetime_event_julday)
		station_file_HHZ.trim(starttime=event_time_UTCdattime-5, endtime=event_time_UTCdattime+30)
		...
		station_file_HHZ.filter('bandpass',freqmin=3, freqmax=5)

		station_file_HHN = read(DIR_DATA+datetime_event_year+'/'+df_sta['network'].values[0]+'/'+sta+'/HHN.D/'+df_sta['network'].values[0]+'.'+sta+'..HHN.D.'+datetime_event_year+'.'+datetime_event_julday)
		station_file_HHN.trim(starttime=event_time_UTCdattime-5, endtime=event_time_UTCdattime+S_P_time+30)
		...
		station_file_HHN.filter('bandpass',freqmin=3, freqmax=5)

		station_file_HHE = read(DIR_DATA+datetime_event_year+'/'+df_sta['network'].values[0]+'/'+sta+'/HHE.D/'+df_sta['network'].values[0]+'.'+sta+'..HHE.D.'+datetime_event_year+'.'+datetime_event_julday)
		station_file_HHE.trim(starttime=event_time_UTCdattime-5, endtime=event_time_UTCdattime+S_P_time+30)
		...
		station_file_HHE.filter('bandpass',freqmin=3, freqmax=5)
...
for times_dictionary in event_dic_time_lst:
	...
	#Saving figure
	folder_name = OUTPUT_FIGURE_DIR+'EVENTS/Local_Picked/Map/'
	...
	folder_name = OUTPUT_FIGURE_DIR+'EVENTS/Local_Picked/Wadati/'
	os.makedirs(folder_name,exist_ok=True)
	...
	for idx,i in enumerate(org_lst):
		...
		data_to_plot.taper(max_percentage=0.2, type="hann")
	...
	axs0.set_xlim(-3,80)
	axs0.set_yticks([])
	axs0.set_title('HHZ (filter: 3-5 Hz)',fontsize=20)
	...
	folder_name = OUTPUT_FIGURE_DIR+'EVENTS/Local_Picked/Seismogram/'
	os.makedirs(folder_name,exist_ok=True)
