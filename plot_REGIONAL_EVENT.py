...
'''
--------------------------------------------------------------------------------
       Function to plot regional dataset according to regional events time
--------------------------------------------------------------------------------
...
Description:
This code will plot the dataset according to a given list of an events.
...
for root, dirs, files in os.walk(OUTPUT_EV_DIR+'Regional/'):
...
for i,j in enumerate(lst_eventsZ):
	...
	output = OUTPUT_EV_DIR+'Regional/NETWORK_MSEED_FILES/'+event_name_info+'/'
	os.makedirs(output,exist_ok=True)
	st_all.write(output+'event_'+event_name_info+'.sac',format='SAC')
...
for i,j in enumerate(lst_eventsZ):
  plot_map_event_data(lst_eventsZ[i],lst_eventsN[i],lst_eventsE[i],EVENT_dir[i],'Regional/',10,20)
...
for i,j in enumerate(lst_eventsX):
    if len(j) != 0:
        plot_map_event_data_hydrophone(lst_eventsX[i],EVENT_dir[i],'Regional/',2,20)
